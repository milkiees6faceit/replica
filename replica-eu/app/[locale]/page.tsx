        ...
        <div className="container grid min-h-[calc(100vh-4rem)] content-center gap-8 py-8 lg:grid-cols-[0.95fr_1.05fr] lg:py-10">
          <div className="animate-rise flex flex-col justify-center">
            <div className="flex flex-wrap gap-2">
              <Badge className="border-0 bg-black px-3 py-1 text-white">{t("badges.drops")}</Badge>
              <Badge className="border-0 bg-[#6C5CE7] px-3 py-1 text-white">{t("badges.suppliers")}</Badge>
              <Badge variant="outline" className="border-black/10 bg-white px-3 py-1">{t("badges.counterfeit")}</Badge>
            </div>
            <h1 className="mt-6 max-w-4xl font-display text-6xl font-black leading-[0.88] tracking-[-0.08em] sm:text-7xl lg:text-8xl">
              {t("hero")} <span className="bg-gradient-to-r from-[#6C5CE7] to-[#A29BFE] bg-clip-text text-transparent">{t("heroSuffix")}</span>
            </h1>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              ...
              <Button asChild size="lg" variant="outline">
                <Link href="#trending">{t("secondaryCta")}</Link>
              </Button>
            </div>
          ...
          <div className="animate-rise-delay grid content-center gap-4">
            ...
            <div className="flex items-center justify-between rounded-[28px] bg-black p-4 text-white sm:p-5">
              <div>
                <p className="text-xs font-black uppercase text-white/50">{t("live.label")}</p>
                <p className="mt-1 text-2xl font-black tracking-[-0.04em]">{t("live.value")}</p>
              </div>
        ...
        <div className="mb-8 flex items-end justify-between gap-4">
          <div>
            <p className="flex items-center gap-2 text-sm font-black uppercase text-[#6C5CE7]">
              <Flame className="h-4 w-4" /> {t("trending.eyebrow")}
            </p>
            <h2 className="mt-2 text-4xl font-black tracking-[-0.06em] md:text-6xl">{t("trending.title")}</h2>
          </div>
          <Button asChild variant="outline" className="hidden sm:inline-flex">
            <Link href={`/${params.locale}/catalog`}>{t("trending.viewAll")}</Link>
          </Button>
        </div>
      ...
      <section className="surface-noise border-y border-black/5 bg-muted py-16">
        <div className="container">
          <div className="mb-8 flex items-end justify-between gap-4">
            <div>
              <p className="flex items-center gap-2 text-sm font-black uppercase text-[#6C5CE7]">
                <Sparkles className="h-4 w-4" /> {t("arrivals.eyebrow")}
              </p>
              <h2 className="mt-2 text-4xl font-black tracking-[-0.06em] md:text-6xl">{t("arrivals.title")}</h2>
            </div>
          </div>
          <div className="grid gap-4 md:grid-cols-3">
            {[
              ["01", t("steps.oneTitle"), t("steps.oneCopy")],
              ["02", t("steps.twoTitle"), t("steps.twoCopy")],
              ["03", t("steps.threeTitle"), t("steps.threeCopy")]
            ].map(([step, title, copy]) => (
          ...
          <div className="flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="flex items-center gap-2 text-sm font-black uppercase text-[#A29BFE]">
                <Lock className="h-4 w-4" /> {t("limited.eyebrow")}
              </p>
            ...
            <Button asChild size="lg" variant="secondary">
              <Link href={`/${params.locale}/catalog`}>{t("limited.cta")}</Link>
            </Button>
          </div>
          <div className="mt-10 grid gap-3 sm:grid-cols-4">
            {[
              [TrendingUp, t("limited.hype")],
              [PackageCheck, t("limited.delivery")],
              [BadgeCheck, t("limited.suppliers")],
              [Zap, t("limited.checkout")]
            ].map(([Icon, label]) => (
